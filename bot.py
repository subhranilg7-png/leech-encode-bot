...
def _aria2_rpc_is_up(host="localhost", port=6800, timeout=1) -> bool:
    try:
...
def ensure_aria2_running():
    """
    Starts aria2c's RPC daemon in the background if it isn't already
    running, so the only command needed to run the bot is `python bot.py`.
    """
    if _aria2_rpc_is_up():
        logger.info("aria2c RPC already running.")
        return

    if shutil.which("aria2c") is None:
        raise RuntimeError(
            "aria2c is not installed. Install it first (e.g. `sudo apt-get install -y aria2`), "
            "then re-run `python bot.py`."
        )

    logger.info("Starting aria2c RPC daemon...")
    subprocess.Popen(
        [
            "aria2c",
            "--enable-rpc",
    ...
    for _ in range(20):  # wait up to ~10s for the RPC port to come up
        if _aria2_rpc_is_up():
